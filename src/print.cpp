...
static void padding(char** dest, const s32 l_flag, const struct params_s *par);
static void outs(char** dest, const charptr lp, struct params_s *par);
...
*
******************************************************************************/
static void outnum(char** dest, const s32 n, const s32 base, struct params_s *par)
{
	s32 negative;
	s32 i;
	char8 outbuf[32];
...
static void outnum1(char** dest, const s64 n, const s32 base, params_t *par)
{
	s32 negative;
	s32 i;
	char8 outbuf[64];
...
try_next:
		...
		switch (tolower(ch)) {
...
#if defined (SUPPORT_64BIT_PRINT)
				if (long_flag != 0) {
					outnum1(dest, (s64)va_arg(argp, s64), 10L, &par);
				} else {
					outnum(dest, va_arg(argp, s32), 10L, &par);
				}
#else
				outnum(dest, va_arg(argp, s32), 10L, &par);
#endif
			...
			case 'p':
				...
				par.unsigned_flag = 1;
				outnum1(dest, (s64)va_arg(argp, s64), 16L, &par);
				Check = 1;
				break;
#endif
			case 'X':
			case 'x':
				...
				if (long_flag != 0) {
					...
					outnum(dest, (s32)va_arg(argp, s32), 16L, &par);
				}
#else
				outnum(dest, (s32)va_arg(argp, s32), 16L, &par);
#endif
			...
			case 's':
				outs(dest, va_arg( argp, char *), &par);
				Check = 1;
				break;
